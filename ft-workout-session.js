/* FORM workout session persistence — per-program offline-first drafts + recovery */
(()=>{'use strict';
const LEGACY='FORM_WORKOUT_DRAFT_V1', PREFIX='FORM_WORKOUT_DRAFT_V2:', LAST='FORM_LAST_WORKOUT_SAVED_V1';
const draftKey=t=>PREFIX+encodeURIComponent(t||'unknown');
const readType=t=>{try{return JSON.parse(localStorage.getItem(draftKey(t))||'null')}catch{return null}};
const write=d=>{try{if(!d?.type)return false;localStorage.setItem(draftKey(d.type),JSON.stringify(d));return true}catch{return false}};
const clearType=t=>{try{localStorage.removeItem(draftKey(t))}catch{}};
const val=id=>document.getElementById(id)?.value??'';
function migrateLegacy(){try{const d=JSON.parse(localStorage.getItem(LEGACY)||'null');if(d?.type&&!readType(d.type))write(d);localStorage.removeItem(LEGACY)}catch{}}
migrateLegacy();
function currentDraft(){return readType(window._wk||'')}
function capture(){
  ...
  const type=window._wk||''; if(!type)return null;
  const prev=readType(type)||{}, date=val('workoutDate')||prev.date||'', exercises=[];
  ...
  const d={version:2,type,date,startedAt,updatedAt:Date.now(),exercises,cardio,rpe:val('ftSessionRpe')};write(d);return d;
}
function hasData(d){return !!d?.exercises?.some(e=>e.note||e.sets?.some(s=>s.weight||s.reps||s.rir||s.done))}
function restore(){
  const d=currentDraft();if(!d||!document.querySelector('[id^="settable_"]'))return;
  ...
  if(hasData(d))injectResumeBadge();
}
function injectResumeBadge(){if(document.getElementById('ftDraftBadge'))return;const first=document.querySelector('.workout-card')?.parentElement||document.querySelector('#app>.card');if(!first)return;const el=document.createElement('div');el.id='ftDraftBadge';el.style='margin:0 0 10px;padding:10px 12px;border:1px solid #cfe0ff;border-radius:12px;background:#f3f7ff;color:#284f86;font-size:12px;font-weight:700';el.textContent='✓ Bu programdaki devam eden girişlerin cihazda kayıtlı.';first.parentElement?.insertBefore(el,first)}
