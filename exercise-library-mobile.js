...
function forceRender(){
  ...
  try{
    ...
    host.innerHTML=rows.length?rows.map(e=>`<div class="ftlib-mobile-row" data-ft-mobile-pick="${esc(e.name)}"><div class="ftlib-mobile-copy"><b>${esc(e.name)}</b><small>${esc(e.category)} • ${esc(e.equipment)} • ${esc(e.primary)}</small></div><span class="ftlib-mobile-select">Seç</span></div>`).join(''):`<div class="ftlib-empty"><b>Sonuç bulunamadı.</b><span>Farklı hareket, kas grubu veya ekipman ara.</span></div>`;
...
function install(){
  ...
  if(overlay.dataset.ftMobileRenderWatch==='3')return;
  overlay.dataset.ftMobileRenderWatch='3';
  ...
  if(results)new MutationObserver(()=>{if(overlay.classList.contains('open')&&!rendering&&!results.querySelector('.ftlib-mobile-row'))setTimeout(forceRender,0)}).observe(results,{childList:true});
  renderBurst();
}
