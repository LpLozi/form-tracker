...
function hydrateExtras(){if(typeof db==='undefined'||!Array.isArray(db.foods))return;const names=new Set(db.foods.map(f=>(f.name||'').toLocaleLowerCase('tr-TR')));let changed=false;extras.forEach(f=>{if(!names.has(f.name.toLocaleLowerCase('tr-TR'))){db.foods.push({...f,sugar:0,sodium:0,potassium:0,calcium:0,iron:0,magnesium:0,vitC:0,vitD:0});changed=true;}});if(changed&&typeof save==='function')save();}
